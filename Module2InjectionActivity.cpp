...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
    ...
    //detect OR value = value injection attack
    if (sql.find("or ") || sql.find("OR ") != std::string::npos)
    {
        std::cout<< "SQL INJECTION DETECTED: " << sql << std::endl; //print error message that injection has been detected
        return false; //if branch is taken, return false
    }

    //detecting value = value injection attack
