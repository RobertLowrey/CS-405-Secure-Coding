...
{
    T result = start;
    T maxLimit = std::numeric_limits<T>::max(); //intializing numericLimit
    T error = std::numeric_limits<T>::min(); //initializing error (overflow)


    for (unsigned long int i = 0; i < steps; ++i)
    {
        if (std::numeric_limits<T>::max() - increment < result) { //if numeric_limits is less than result, return error
            return error; //overflow has occurred
        }
        else { //else return result plus increment
...
T subtract_numbers(T const& start, T const& decrement, unsigned long int const& steps) {
    ...
    T result = start;
    T error = std::numeric_limits<T>::max(); // initializing error (overflow)
    T minLimit = 0; //setting minLimit to 0 


    for (unsigned long int i = 0; i < steps; ++i)
    {
        if (minLimit + decrement > result) { //if minLimit is greater than result, return error
            return error; //overflow has occurred
        }
        else { //else return result minus decrement
            result -= decrement;
        }
    }
...
void test_overflow()
{
    ...
    if (result == std::numeric_limits<T>::min()) { // check for overflow
        std::cout << "Overflow detected!" << std::endl; //display overflow has occured
    }
    else {
...
void test_underflow()
{
    ...
    if (result == std::numeric_limits<T>::max()) { // check for overflow
        std::cout << "Underflow detected!" << std::endl; //display overflow has occured
    }
    else {
        std::cout << +result << std::endl;
    }
